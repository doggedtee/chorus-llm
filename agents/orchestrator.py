...
from core.agent_logger import AgentLogger, log_routing
...
async def run_decomposition(context: SharedContext) -> SharedContext:
    ...
    await logger.end(
        ...
        token_count=context.token_usage.get("decomposition", 0),
    )
    return context
...
async def run_retrieval(context: SharedContext) -> SharedContext:
    ...
    await logger.end(
        ...
        token_count=context.token_usage.get("retrieval", 0),
    )
    return context
...
async def run_critique(context: SharedContext) -> SharedContext:
    ...
    await logger.end(
        ...
        token_count=context.token_usage.get("critique", 0),
    )
    return context
...
async def run_synthesis(context: SharedContext) -> SharedContext:
    ...
    await logger.end(
        ...
        token_count=context.token_usage.get("synthesis", 0),
    )
